...
create view vue_effectif_complet as select effectif.id, effectif.genre, effectif.nom, effectif.prenom, instruments.groupe, instruments.nomcomplet as fonction, date_format(effectif.naissance, '%d-%m-%Y') as naissance,  timestampdiff(year, effectif.naissance, curdate()) as age, date_format(effectif.recrutement, '%d-%m-%Y') as recrutement, timestampdiff(year, effectif.recrutement, curdate()) as anciennete, pays.nom as pays, effectif.ville, effectif.courriel from effectif
...
create view vue_effectif_operationnel as select effectif.id, effectif.genre, effectif.nom, effectif.prenom, instruments.groupe, instruments.nomcomplet as fonction,    timestampdiff(year, effectif.naissance, curdate()) as age,  timestampdiff(year, effectif.recrutement, curdate()) as anciennete,  pays.nom as pays, effectif.ville, effectif.courriel from effectif
left join genres on (effectif.genre = genres.sexe)
left join instruments on (effectif.fonction = instruments.nom)
left join pays on (effectif.nationalite = pays.iso)
...
create view vue_effectif_chronologique_desc as select effectif.id, effectif.genre, effectif.nom, effectif.prenom, instruments.groupe, instruments.nomcomplet as fonction,  effectif.ville, pays.nom as pays,  timestampdiff(year, effectif.naissance, curdate()) as age,    timestampdiff(year, effectif.recrutement, curdate()) as anciennete, effectif.courriel from effectif
left join genres on (effectif.genre = genres.sexe)
left join instruments on (effectif.fonction = instruments.nom)
left join pays on (effectif.nationalite = pays.iso)
having age between 18 and 65
...
create view vue_effectif_retraite as select  effectif.id, genres.civilite, effectif.nom, effectif.prenom, pays.nom as pays, instruments.groupe, instruments.nomcomplet as fonction,  date_format(effectif.naissance, '%d-%m-%Y') as naissance, timestampdiff(year, effectif.naissance, curdate()) as age,   timestampdiff(year, effectif.recrutement, curdate()) as anciennete
from effectif
left join genres on (effectif.genre = genres.sexe)
left join instruments on (effectif.fonction = instruments.nom)
left join pays on (effectif.nationalite = pays.iso)
...
select id, pays, genre, nom, prenom,  groupe, fonction, anciennete  from
...
select id, nom, prenom,  groupe, fonction, ville, pays, age, anciennete from
...
create view vue_lecture_id as select id, genre, nom, prenom, groupe, fonction, naissance, recrutement, pays, ville, courriel  from vue_effectif_complet;

drop view if exists vue_lecture_mod;
create view vue_lecture_mod as select id, genre, effectif.nom, prenom, instruments.groupe, instruments.nom as fonction, date_format(effectif.naissance, '%d-%m-%Y') as naissance, pays.iso as iso, ville, courriel, date_format(effectif.recrutement, '%d-%m-%Y') as recrutement from effectif
left join instruments on (effectif.fonction = instruments.nom)
left join pays on (effectif.nationalite = pays.iso);
...
create view vue_base_instrumentale as select effectif.id, effectif.genre, genres.civilite, effectif.nom, effectif.prenom, instruments.groupe, instruments.nom as abreviation, instruments.nomcomplet as fonction,   date_format(effectif.naissance, '%d-%m-%Y') as naissance, timestampdiff(year, effectif.naissance, curdate()) as age,  date_format(effectif.recrutement, '%d-%m-%Y') as recrutement, timestampdiff(year, effectif.recrutement, curdate()) as anciennete, pays.nom as pays, effectif.ville, effectif.courriel from effectif
left join genres on (effectif.genre = genres.sexe)
left join instruments on (effectif.fonction = instruments.nom)
left join pays on (effectif.nationalite = pays.iso)
having age between 18 and 65
...
select id, civilite, nom, prenom, age, anciennete, pays
...
select id, civilite, nom, prenom, naissance, pays, age, anciennete
...
select effectif.id, genres.sexe, effectif.nom, effectif.prenom, date_format(effectif.naissance, '%d-%m-%Y') as naissance, timestampdiff(year, effectif.naissance, curdate()) as age, pays.nom as pays, instruments.nomcomplet as fonction, timestampdiff(year, effectif.recrutement, curdate()) as anciennete
from effectif
left join genres on (effectif.genre = genres.sexe)
left join instruments on (effectif.fonction = instruments.nom)
left join pays on (effectif.nationalite = pays.iso)
